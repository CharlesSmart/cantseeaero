...
const ProfileManager: React.FC<ProfileManagerProps> = ({ profiles, onSelectProfile, selectedProfileId, onDeleteProfile }) => {
  ...
  const selectedDisplayId = selectedProfile?.displayId || 1;
  const selectedProfileIdString = toWords(selectedDisplayId);
      ...
      <Tabs value={selectedProfileIdString || ""}>
        ...
        <TabsList>
          {profiles.map((profile, index) => {
            const displayId = profile.displayId || (index + 1);
            const displayText = capitalize(toWords(displayId));
            ...
            return (
              ...
              >
                <div className="h-10 w-10 rounded-md bg-secondary overflow-hidden">
                  {profile.imageUrl && <img src={profile.cachedImageUrl || profile.imageUrl || ''} alt={`Profile ${displayId} preview`} className="h-full object-cover" />}
                </div>

                <div className='min-w-10 text-left'>{displayText}</div>
                ...
                <div className='flex grow justify-end'>
                  <div 
                    ...
                    aria-label={`Delete profile ${displayText}`}
