...
function App() {
  ...
  const handleRemoveBG = async () => {
    if (selectedProfile) {
      ...
      try {
        const maskImage = await removeBG(selectedProfile);
        const updatedProfile = {
          ...selectedProfile,
          maskImage, // Assuming maskImage is a property in Profile
        };
        handleUpdateProfile(updatedProfile);
      } catch (error) {
  ...
  return (
    ...
    <div className="min-w-[100vw] min-h-screen bg-background-pattern bg-no-repeat bg-cover">
        ...
        {profiles.some(profile => profile.uploadedImage !== null) &&
          ...
          {/* Display the mask image to be fixed*/}
          {selectedProfile?.maskImageUrl && (
            <div className="mask-image-container">
              <img src={(selectedProfile.maskImageUrl)} alt="Mask" />
            </div>
          )}
          </>
        }
