...
function App() {
  ...
  const [globalMeasurementPixels, setGlobalMeasurementPixels] = useState<number | null>(null);
  const [globalMeasurementMm, setGlobalMeasurementMm] = useState<number | null>(null);
  ...
  useEffect(() => {
    const loadProfiles = async () => {
      ...
      setGlobalMeasurementPixels(profilesWithBlobUrls[0].measurementPixels);
      setGlobalMeasurementMm(profilesWithBlobUrls[0].measurementMm);
      } else {
  ...
  const handleRulerUpdate = (pixels: number) => {
    if (selectedProfile) {
      ...
      handleUpdateProfile(updatedProfile);
      setGlobalMeasurementPixels(pixels); // Update global value
    }
  };
  ...
  const handleLengthUpdate = (length: number) => {
    if (selectedProfile) {
      ...
      handleUpdateProfile(updatedProfile);
      setGlobalMeasurementMm(length); // Update global value
    }
    ...
    <div className="min-w-screen min-h-screen bg-background-pattern bg-no-repeat bg-cover">
        ...
        {profiles.some(profile => profile.uploadedImage !== null) &&
          ...
          <AnalysisPanel 
            ...
            measurementPixels={selectedProfile?.measurementPixels ?? null}
            measurementMm={globalMeasurementMm ?? null}
