...
function App() {
  ...
  useEffect(() => {
    const loadProfiles = async () => {
      ...
      const profilesWithBlobUrls = storedProfiles.map(profile => {
        const uploadedImageUrlfromIDB = profile.uploadedImageUrl ? URL.createObjectURL(new Blob([profile.uploadedImage])) : null;
        const cachedImageUrlfromIDB = profile.cachedImageUrl ? URL.createObjectURL(new Blob([profile.cachedImage])) : null;
        return {
          ...profile,
          uploadedImage: uploadedImageUrlfromIDB,
          cachedImage: cachedImageUrlfromIDB,
        };
      });
  ...
  const handleUpdateProfile = async (updatedProfile: Profile) => {
    setProfiles(profiles.map(profile => 
      ...
      console.log("update profile", updatedProfile.uploadedImage)
    ));
  ...
  useEffect(() => {
    return () => {
      profiles.forEach(profile => {
        if (profile.uploadedImage) URL.revokeObjectURL(profile.uploadedImage);
        if (profile.cachedImage) URL.revokeObjectURL(profile.cachedImage);
      });
    };
  }, [profiles]);
