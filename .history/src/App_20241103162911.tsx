...
function App() {
  ...
  return (
    <div className="min-w-screen min-h-screen bg-background-pattern bg-no-repeat bg-cover">
        <>
        {!selectedProfile.uploadedImage &&
          <ImageUploader onImageUpload={handleImageUpload} uploadedImage={selectedProfile?.uploadedImage as File} imageUrl={selectedProfile?.imageUrl ?? ''}/>
        }
        {selectedProfile?.uploadedImage &&
          <>
          <PixelCounter 
