...
const MeasurementTool: React.FC<MeasurementToolProps> = ({ imageUrl, onRulerUpdate, onRemoveBG }) => {
  ...
  const handleCanvasMouseMove = useCallback((event: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (startPoint && endPoint) {
      if (
        isNearLine(x, y) ||
        Math.sqrt(Math.pow((x - startPoint.x)/zoomLevel, 2) + Math.pow((y - startPoint.y)/zoomLevel, 2)) < 10 ||
        Math.sqrt(Math.pow((x - endPoint.x)/zoomLevel, 2) + Math.pow((y - endPoint.y)/zoomLevel, 2)) < 10
      ) {
        setCursorStyle('move');
      } else {
