...
const EmptyState: React.FC<EmptyStateProps> = ({ onImageUpload, uploadedImage, onLoadDemoProfiles}) => {
  return (
    <Card className='max-w-xs fixed left-1/2 -translate-x-1/2 top-1/2 -translate-y-1/2'>
        ...
        <a className='flex flex-row gap-2 group cursor-pointer mt-4' onClick={onLoadDemoProfiles}>
            <div className='border bg-card rounded-md -rotate-2 translate-x-4 translate-y-2 transition-all group-hover:-rotate-3 group-hover:translate-y-1'><img src="/demo/examplepreview.png" srcSet="/demo/examplepreview@2x.png 2x" alt="Example trainer image"  /></div>
            <div className='border bg-card rounded-md rotate-2 -translate-x-4 translate-y-2 transition-all group-hover:rotate-3 group-hover:-translate-y-1'><img src="/demo/examplepreview.png" srcSet="/demo/examplepreview@2x.png 2x" alt="Example trainer image"  /></div>
        </a>
      </CardContent>
    </Card>
