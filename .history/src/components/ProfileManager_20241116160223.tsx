          ...
          {profiles.map(profile => (
            ...
            >
              ...
              <div className='grid grid-cols-3'>
                <div><div className="h-2 w-2 rounded-full bg-primary"></div>{Math.round(profile.pixelCounts?.opaquePercentage ?? 0)}%</div>
                <div><div className="h-2 w-2 rounded-full bg-primary opacity-50"></div>{Math.round(profile.pixelCounts?.semiTransparentPercentage ?? 0)}%</div>
                <div><div className="h-2 w-2 rounded-full bg-none border border-primary"></div>{Math.round(profile.pixelCounts?.transparentPercentage ?? 0)}%</div>
