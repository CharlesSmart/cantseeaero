...
const MeasurementTool: React.FC<MeasurementToolProps> = ({ imageUrl, onRulerUpdate, onRemoveBG, isBGRemovalLoading }) => {
  ...
  const drawMeasurementTool = useCallback(() => {
    ...
    if (startPoint && endPoint) {
      ...
      const leftPoint = Math.min(startPoint.x, endPoint.x);
      const rightPoint = Math.max(startPoint.x, endPoint.x);
      ...
      ctx.rotate(Math.atan2(endPoint.y - startPoint.y, rightPoint - leftPoint));
