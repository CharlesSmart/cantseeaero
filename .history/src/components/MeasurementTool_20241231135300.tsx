...
import { useProfileComparisonContext } from './ProfileComparison';
...
const MeasurementTool: React.FC<MeasurementToolProps> = ({ imageUrl, onRulerUpdate, onRemoveBG, isBGRemovalLoading, selectedProfileId, profiles }) => {
  ...
  const { imageUrlComparison } = useProfileComparisonContext();
  const canvasRefCompare = useRef<HTMLCanvasElement | null>(null);

  useEffect(() => {
    const canvas = canvasRefCompare.current;
    if (canvas) {
      const ctx = canvas.getContext('2d');
      if (ctx) {
        if (imageUrlComparison) {
          const img = new Image();
          img.src = imageUrlComparison;
          img.onload = () => {
  ...
  return (
    ...
    <div>
      {imageUrl && ( 
        <>
          <canvas ref={canvasRefCompare} width={100} height={100} />
