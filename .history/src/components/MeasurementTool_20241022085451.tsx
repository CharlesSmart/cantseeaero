...
const MeasurementTool: React.FC<MeasurementToolProps> = ({ imageUrl, onRulerUpdate, onRemoveBG }) => {
  ...
  const calculateDistance = useCallback((start: Point, end: Point) => {
    return Math.round(Math.sqrt(Math.pow((end.x - start.x) / zoomLevel, 2) + Math.pow((end.y - start.y) / zoomLevel, 2)));
  }, [zoomLevel]);
  ...
  const drawMeasurementTool = useCallback(() => {
    ...
    if (startPoint && endPoint && targetImg.current) {
      ...
      ctx.drawImage(targetImg.current, startPoint.x - 10, startPoint.y - 10, 20, 20);
      ctx.drawImage(targetImg.current, endPoint.x - 10, endPoint.y - 10, 20, 20);
    }

    ctx.restore();
  }, [startPoint, endPoint, zoomLevel]);
  ...
  const handleCanvasMouseMove = useCallback((event: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (startPoint && endPoint) {
      if (
        isNearLine(x, y) ||
        Math.sqrt(Math.pow(x - startPoint.x, 2) + Math.pow(y - startPoint.y, 2)) < 10 ||
        Math.sqrt(Math.pow(x - endPoint.x, 2) + Math.pow(y - endPoint.y, 2)) < 10
      ) {
        setCursorStyle('move');
      } else {
    ...
    <div>
      {imageUrl && ( // Check if imageUrl is provided
        <>
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild><Button variant="ghost">{zoomLevel*100}%<ChevronDown className="mx-l-2 w-4 h-4" /></Button></DropdownMenuTrigger>
            <DropdownMenuContent>
