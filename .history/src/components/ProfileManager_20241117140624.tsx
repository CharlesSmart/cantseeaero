        ...
        <TabsList>
          ...
          {profiles.map(profile => (
            ...
            >
              ...
              <div className='min-w-10'>{capitalize(toWords(profile.id))}</div>
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <div className='grid grid-cols-3 gap-1 text-muted-foreground font-mono text-xs'>
                        <div className='flex items-center gap-1'><div className="h-2 w-2 rounded-full bg-primary"></div>{Math.round(profile.pixelCounts?.opaquePercentage + profile.pixelCounts?.semiTransparentPercentage ?? 0)}%</div>
                    ...
                    <TooltipContent className='max-w-sm text-left'>
                      <p>Pixel analysis:<br />Opaque: {profile.pixelCounts?.opaquePercentage + profile.pixelCounts?.semiTransparentPercentage ?? 0}%<br />Transparent: {profile.pixelCounts?.transparentPercentage ?? 0}%</p>
                    </TooltipContent>
                  </Tooltip>
                  </TooltipProvider>   
              }
