...
const EmptyState: React.FC<EmptyStateProps> = ({ onImageUpload, uploadedImage, onLoadDemoProfiles}) => {
  return (
    <Card className='max-w-xs fixed left-1/2 -translate-x-1/2 top-1/2 -translate-y-1/2'>
        ...
        <CardContent> 
        <ImageUploader className='border-none h-auto mb-2' onImageUpload={onImageUpload} uploadedImage={uploadedImage} />
        <p className='text-sm text-muted-foreground text-center'>or try a demo example</p>    
        <a className='flex flex-row gap-2 group cursor-pointer mt-8' onClick={onLoadDemoProfiles}>
