...
import {
  ChevronDown, Eraser, MousePointer2, Ruler
} from "lucide-react"
...
const MeasurementTool: React.FC<MeasurementToolProps> = ({ imageUrl, onRulerUpdate, onRemoveBG }) => {
  ...
  const [cursorStyle, setCursorStyle] = useState<string>('default | crosshair');
  ...
  const handleCanvasMouseDown = (event: React.MouseEvent<HTMLCanvasElement>) => {
    if (selectedTool !== 'ruler') return; // Only proceed if the ruler tool is selected
  ...
  }, [startPoint, endPoint, isDragging, zoomLevel, calculateDistance, drawMeasurementTool, selectedTool]);
  ...
  return (
    <div className='bg-gradient-to-t from-blue-50 to-gray-50'>
