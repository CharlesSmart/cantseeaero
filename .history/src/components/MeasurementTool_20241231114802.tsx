...
const MeasurementTool: React.FC<MeasurementToolProps> = ({ imageUrl, onRulerUpdate, onRemoveBG, isBGRemovalLoading, selectedProfileId, profiles }) => {
  ...
  const drawMaskImage = useCallback(() => {
    if (!maskCanvasRef || !imageRef.current || selectedProfileId === null) return;

    const selectedProfile = profiles.find(profile => profile.id === selectedProfileId);
    if (!selectedProfile || !selectedProfile.maskImageUrl) return;
    ...
    maskImage.onload = () => {
      maskCanvasRef.width = imageRef.current.width * zoomLevel;
      maskCanvasRef.height = imageRef.current.height * zoomLevel;
      maskCtx.drawImage(maskImage, 0, 0, maskCanvasRef.width, maskCanvasRef.height);
    };
    maskImage.src = selectedProfile.maskImageUrl;
  }, [maskCanvasRef, zoomLevel, profiles, selectedProfileId]);
