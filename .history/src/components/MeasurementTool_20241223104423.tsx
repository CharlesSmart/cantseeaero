...
const MeasurementTool: React.FC<MeasurementToolProps> = ({ imageUrl, onRulerUpdate, onRemoveBG, isBGRemovalLoading, selectedProfileId, profiles }) => {
  ...
  const handleCanvasMove = useCallback((event: React.MouseEvent<HTMLCanvasElement> | React.TouchEvent<HTMLCanvasElement>) => {
    const { x, y } = getEventCoordinates(event);
    const canvas = canvasRef.current;
    if (!canvas) return;

    if (selectedTool === 'move' && isImageDragging) {
      const dx = ('movementX' in event ? event.movementX : event.changedTouches[0].clientX - event.touches[0].clientX) / zoomLevel;
      const dy = ('movementY' in event ? event.movementY : event.changedTouches[0].clientY - event.touches[0].clientY) / zoomLevel;
