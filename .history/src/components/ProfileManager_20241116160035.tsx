          ...
          {profiles.map(profile => (
            ...
            >
              ...
              <div className='grid grid-cols-3'>
                <span><span className="h-2 w-2 rounded-full bg-primary"></span>{Math.round(profile.pixelCounts?.opaquePercentage ?? 0)}%</span>
                <span><span className="h-2 w-2 rounded-full bg-secondary"></span>{Math.round(profile.pixelCounts?.transparentPercentage ?? 0)}%</span>
                <span><span className="h-2 w-2 rounded-full bg-muted"></span>{Math.round(profile.pixelCounts?.semiTransparentPercentage ?? 0)}%</span>
              </div>
              }
