...
const ProfileManager: React.FC<ProfileManagerProps> = ({ profiles, onSelectProfile, selectedProfileId, onDeleteProfile }) => {
  ...
  const { setImageUrlComparison } = useProfileComparisonContext();

  const handleMouseEnter = (imageUrl: string) => {
    setImageUrlComparison(imageUrl);
  };

  const handleMouseLeave = () => {
    setImageUrlComparison('');
  };

  return (
    <div>
      <canvas ref={canvasRef} width={100} height={100} className="mb-4"></canvas>
