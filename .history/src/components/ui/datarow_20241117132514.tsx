...
export const DataRowWithInput: React.FC<{
    ...
    disabled?: boolean;
    unit?: string;
}> = ({ label, value, onBlur, disabled, unit }) => {
    return (
        <div className='grid grid-cols-2 gap-8 text-sm text-secondary-foreground items-center'>
            <p>{label}</p>
            <div className='grid items-center'>
            <Input value={value !== undefined ? value : ''} onBlur={onBlur} className='w-20 justify-self-end font-mono disabled:text-secondary-foreground disabled:opacity-100' disabled={disabled} />
