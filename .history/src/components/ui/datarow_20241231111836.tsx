        ...
        <div className={`grid grid-cols-2 gap-8 text-sm  items-center`}>
            ...
            <div className='grid items-center'>
                <Input
                    ...
                    value={inputValue}
                    onChange={onChange}
                    className={`w-20 justify-self-end font-mono disabled:text-secondary-foreground disabled:opacity-100 ${variant === 'priority' ? 'bg-blue-50 border-none text-blue-600' : 'bg-card'}`}
                    disabled={disabled}
                />
                {unit && <p className={`absolute right-8 text-right text-muted-foreground ${variant === 'priority' ? 'text-blue-600' : 'text-muted-foreground'}`}>{unit}</p>}
            </div>
        </div>
    );
};
