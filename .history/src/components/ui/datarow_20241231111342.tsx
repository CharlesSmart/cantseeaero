...
export const DataRowWithInput: React.FC<{
    ...
    unit?: string;
}> = ({ label, value, onChange, readonly, unit, variant }) => {
        ...
        <div className={`grid grid-cols-2 gap-8 text-sm  items-center`}>
            ...
            <div className='grid items-center'>
                <Input
                    ...
                    value={inputValue}
                    onChange={onChange}
                    className={`w-20 justify-self-end font-mono disabled:text-secondary-foreground disabled:opacity-100 ${variant === 'priority' ? 'bg-primary-foreground ' : 'bg-card'}`}
                    readOnly={readonly}
                />
