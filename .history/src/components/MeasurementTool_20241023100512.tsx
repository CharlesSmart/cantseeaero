...
const MeasurementTool: React.FC<MeasurementToolProps> = ({ imageUrl, onRulerUpdate, onRemoveBG }) => {
  ...
  const drawMeasurementTool = useCallback(() => {
    ...
    if (startPoint && endPoint && targetImg.current) {
      ...
      ctx.rect(startPoint.x-baseSize, startPoint.y - baseSize, baseSize/10, baseSize);
      ...
      // Draw draggable points with scaled size
     
      ctx.drawImage(
        targetImg.current,
        endPoint.x - scaledSize / 2,
        endPoint.y - scaledSize / 2,
        scaledSize,
        scaledSize
      );
    }
