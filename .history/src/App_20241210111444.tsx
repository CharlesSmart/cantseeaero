...
function App() {
  ...
  const handleRulerUpdate = async (pixels: number) => {
    ...
    // Use Promise.all to ensure all updates are completed
    await Promise.all(updatedProfiles.map(profile => handleUpdateProfile(profile)));
  };
